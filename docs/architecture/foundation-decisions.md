...
Implemented Dice, balance, Points Exchange, fairness, and Daily Claim BFF ownership:
...
src/app/api/balance/watch-to-game/route.ts POST /api/balance/watch-to-game
...
src/features/points-exchange/** Points Exchange local client, TanStack Query mutation wiring, browser-safe types, and modal UI.
```
...
Points Exchange is an authenticated TopBar balance action. TopBar owns the balance popover anchor and modal open state, while `src/features/points-exchange/**` owns exchange request/response types, the browser-safe local client, mutation wiring, validation-facing modal UI, and successful cache application. The browser client calls only local `POST /api/balance/watch-to-game` with a positive integer number; backend Decimal-string adaptation is server-side in the BFF only. On success, the mutation uses the backend-authored final balances from the BFF response to immediately update the existing `balanceQueryKey` cache in the shared `Balance` shape; it does not introduce a new balance store or browser-side balance authority. The exchange modal accepts positive integer amounts only, keeps the popover action available even when Watch Points are `0`, and shows insufficient-balance or validation errors inline rather than adding toast UX.
...
`SoundProvider` mounts in the Game Detail shell parallel to `TurboModeProvider`, keyed by `game.slug`. It mirrors the `TurboModeProvider`/`useTurboMode` contract shape and missing-provider error behavior. `SoundService` is an SSR-safe singleton (guards `typeof window`, lazily constructs `Howl` instances only on first `play()`) and discards `play()` calls silently when the AudioContext is not yet unlocked rather than queuing them. muted/volume preferences persist to `localStorage` under `sound:prefs:v1`, mirroring the `roulette:bets:v1` persistence pattern. The global volume slider and mute toggle in `src/widgets/game-detail/game-actions.tsx` are connected to `useSoundContract()`.

Sound shell slice 2, pass 1 (UI layer audible) updates the `SoundEvent` vocabulary to the final 11 events, fills the `SOUND_REGISTRY` with real `/sounds/**` paths served from `public/`, and wires the first per-game and per-control emissions. The final `SoundEvent` union is:

```txt
ui:click | ui:tick
bet:win
dice:throw | dice:rolling | dice:score
keno:select | keno:reveal | keno:match
plinko:drop | plinko:pocket
roulette:spin
```
