...
// ─── Daily Claimer chip icon (src/shared/assets/nav/icons/daily-claimer-chip.svg) ──

function DailyClaimerChipIcon({ className }: { className?: string }) {
  return (
    <svg
      className={className}
      width="14"
...
// ─── Daily Claimer (expanded) ─────────────────────────────────────────────────

function DailyClaimer() {
  return (
    <div
    ...
    >
      {/* Dark base */}
      <div className="absolute inset-0" style={{ background: "#09110e" }} />
      ...
      {/* Claim button */}
      <div className="absolute bottom-[12px] left-[12px] flex h-[32px] cursor-pointer items-center gap-[4px] rounded-[8px] bg-gradient-to-b from-primary-tint to-primary px-[12px] py-[6px] text-sm font-medium text-on-primary">
        <span>Claim</span>
        <div className="flex items-center gap-[2px]">
          <DailyClaimerChipIcon className="h-[14px] w-[14px] shrink-0" />
          <span>10</span>
        </div>
      </div>
    </div>
...
// ─── Caret icon for Games collapsible trigger ─────────────────────────────────
...
interface MainNavProps {
  collapsed: boolean;
  /** Called when collapsed nav receives a Games trigger click — lets the parent expand the nav. */
