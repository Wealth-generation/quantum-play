...
import { createCanvasPlinkoRenderer } from "../renderer/canvas/canvas-plinko-renderer";
import type {
...
export function PlinkoCanvasStage({
    ...
    (
      ...
      console.debug(
        `[Plinko playback:${playbackId ?? `plinko-stage-${round.id}`}] ${phase}`,
        {
          ...
          terminal,
          turboEnabled: round.turboEnabled,
        },
      );
    },
    [],
  ...
  React.useEffect(() => {
    ...
    void createCanvasPlinkoRenderer({ container })
      .then((renderer) => {
        if (cancelled) {
          renderer.destroy();
          return;
      ...
      .catch(() => {
        ...
        rendererInitFailureRef.current = "canvas-renderer-init-failed";
        visualizeQueuedRounds();
      });
