...
} as const satisfies PlinkoMultiplierTable;

export function isPlinkoRisk(value: unknown): value is PlinkoRisk {
  return typeof value === "string" && PLINKO_RISKS.includes(value as PlinkoRisk);
}
...
export function isPlinkoRows(value: unknown): value is PlinkoRows {
  return (
    ...
    PLINKO_ROWS.includes(value as PlinkoRows)
  );
}
...
export function getPlinkoMultiplier(
  ...
  return plinkoMultipliers[risk][rowsCount][bucketIndex] ?? null;
}
