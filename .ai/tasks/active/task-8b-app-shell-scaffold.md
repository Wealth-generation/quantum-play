...
- Fix: changed `flex` → `hidden lg:flex` on the header row `<div>`. On mobile (`< lg`) the
  ...
  meaningful for the persistent sidebar). Nav items, Games Collapsible, daily-claimer
  placeholder, and Help & Support pinned row are unaffected in both contexts.
